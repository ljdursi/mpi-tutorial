...
void bufferGuardcells(float ***u, const int nx, const int ny, 
    ...
    for (j=NGUARD; j<ny-NGUARD; j++) {
        for (i=nx-2*NGUARD; i<nx-NGUARD; i++) {
            for (v=0; v<NVARS; v++) {
                sendbuffer[count] = u[j][i][v];
                count++;
            }
        }
    }
    MPI_Sendrecv(sendbuffer, datasize,  MPI_FLOAT, right, righttag,
                 recvbuffer, datasize,  MPI_FLOAT, left,  righttag, cartcomm, &rstatus);
    count = 0;
    for (j=NGUARD; j<ny-NGUARD; j++) {
        for (i=0; i<NGUARD; i++) {
            for (v=0; v<NVARS; v++) {
                u[j][i][v] = recvbuffer[count];
                count++;
            }
        }
    }
    ...
    MPI_Sendrecv(sendbuffer, datasize,  MPI_FLOAT, left,  lefttag,
                 recvbuffer, datasize,  MPI_FLOAT, right, lefttag, cartcomm, &rstatus);
    count = 0;
    for (j=NGUARD; j<ny-NGUARD; j++) {
        for (i=nx-NGUARD;i<nx; i++) {
            for (v=0; v<NVARS; v++) {
                u[j][i][v] = recvbuffer[count];
                count++;
            }
...
void vectorGuardcells(float ***u, const int nx, const int ny, const char xydim,
    ...
    ierr = MPI_Sendrecv(&(u[0][nx-2*NGUARD][0]), 1, xbctype, right, righttag, 
                        &(u[0][0][0]), 1, xbctype, left, righttag, 
                        cartcomm, &rstatus);

    ierr = MPI_Sendrecv(&(u[0][NGUARD][0]), 1, xbctype, left, lefttag, 
                        &(u[0][nx-NGUARD][0]), 1, xbctype, right, lefttag, 
                        cartcomm, &rstatus);
